        ...
        try{
            ...
            if ($columnaBusqueda == "" || $valorBusqueda == "") {
                ...
                $stmt = $this->_DB->prepare ("	SELECT id, pedido, cliente, cedtecnico, tecnico, accion, asesor, fecha, duracion, region, idllamada, observacion
						...
						");
                $stmt->execute([':fechaini' => "$fechaini 00:00:00", ':fechafin' => "$fechafin 23-59-59",':page'=>"$page"]);
                ...
                $stmt = $this->_DB->prepare  ("	SELECT id, pedido, cliente, cedtecnico, tecnico, accion, asesor, fecha, duracion, region, idllamada, observacion
								FROM quejasgo
									...
									limit 100 offset :page
						");
                $stmt->execute([':fechaini' => "$fechaini 00:00:00", ':fechafin' => "$fechafin 23-59-59",':columnaBusqueda'=>"$columnaBusqueda",':valorBusqueda'=>"$valorBusqueda",':page'=>"$page"]);
