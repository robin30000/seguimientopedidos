...
class kpi
{
    ...
    public function contigenciaHoraAgente($data)
    {
		...
		WHERE 1=1 AND acepta = 'acepta' AND p.horacontingencia BETWEEN '$today 00:00:00' AND '$today 23:59:59') C2
		GROUP BY C2.USUARIO, C2.prod ORDER BY CANTIDAD DESC");
    ...
    public function contigenciaHoraAgenteApoyo($data)
    {
		...
		SELECT 
				 ...
				 p.logincontingencia AS USUARIO, DATE_FORMAT(p.horacontingencia, '%H') AS RANGO_PENDIENTE
		FROM contingencias p
		WHERE p.logincontingencia in ('caro','jalvarga','alondono','karroyar','jbetanmo','larrublg', 'cramiceb','jromang','cvasquor','garcila','gdbetancur','jcordoba') $condicion AND p.horacontingencia BETWEEN '$today 00:00:00' AND '$today 23:59:59') C2
    ...
    public function contigenciaHoraAgenteTiempoCompleto($data)
    {
		...
		SELECT 
				 ...
				 p.logincontingencia AS USUARIO, DATE_FORMAT(p.horacontingencia, '%H') AS RANGO_PENDIENTE
		FROM contingencias p
		WHERE p.logincontingencia in ('jricol','lgarzon','cbarrera','jaceveb','jechavs') $condicion AND p.horacontingencia BETWEEN '$today 00:00:00' AND '$today 23:59:59') C2
		GROUP BY C2.USUARIO ORDER BY CANTIDAD DESC");
