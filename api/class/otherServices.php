...
class otherServices
{
    ...
    public function insertarCambioEquipo($data)
    {
        try {
            ...
            if ($tecnologia == "DTH") {
                ...
                if ($stmt->rowCount() == 1) {
                    $response = array('state' => 1, 'msj' => 'Cambios realizados correctamente');
                } else {
    ...
    public function ciudades(){
        try {
            $stmt = $this->_DB->prepare("SELECT * FROM ciudad_contingencia");
            $stmt->execute();

            if ($stmt->rowCount() > 0){
                $response = array('state' => true, 'data' => $stmt->fetchAll(PDO::FETCH_ASSOC));
            }else{
                $response = array('state' => false, 'msj' => 'No se encontraron registros');
            }
            $this->_DB = '';
            return $response;
