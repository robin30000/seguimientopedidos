...
class GestionQuejasGo
{
    ...
    public function marcarEnGestionQuejasGo($data)
    {
        try {
			...
			$stmt = $this->_DB->prepare("SELECT en_gestion, pedido, login_gestion FROM ventasInstaleTiendas WHERE id = :id");
			$stmt->execute(array(':id' => $id));
			$response = $stmt->fetchAll(PDO::FETCH_ASSOC);

			if ($response[0]['en_gestion'] == 0) {
				$stmt = $this->_DB->prepare("UPDATE ventasInstaleTiendas SET en_gestion = 1, login_gestion = :login_gestion WHERE id = :id");
				...
				$stmt = $this->_DB->prepare("UPDATE ventasInstaleTiendas SET en_gestion = 0, login_gestion = '' WHERE id = :id");
				$stmt->execute(array(':id' => $id));
				if ($stmt->rowCount() == 1) {
