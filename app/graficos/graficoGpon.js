(function () {
    "use strict";
    angular.module("seguimientopedidos").controller("GraficoGponCtrl", GraficoGponCtrl);
    GraficoGponCtrl.$inject = ["$scope", "$http", "$rootScope", "$route", "services", "$filter"];

    function GraficoGponCtrl($scope, $http, $rootScope, $route, services) {
        ...
        $scope.buscar = (data) => {
            if (!data.fecha) {
                Swal({
                    type: 'error',
                    title: 'Opss...',
                    text: 'debes ingresar la fecha de consulta',
                    timer: 4000
                })
                return;
            }
        ...
        function graphicDetails(data) {
            ...
            services.myService(datos, 'soporteGponCtrl.php', 'graphicDetails')
                .then((respuestas) => {
                    if (respuestas.data.state) {
                        ...
                        $scope.myChart1 = new Chart(ctx1, {
                            ...
                            options: {
                                title: {
                                    display: true,
                                    text: 'Grafico GPON (Consolidado: ' + total + ')',
                                    position: 'top',
                                },
                                plugins: {
                                    datalabels: {
        ...
        function gestionPorHora(data) {
            ...
            services.myService(datos, 'soporteGponCtrl.php', 'gestionPorHora').then((data) => {
                $scope.loading = 0;
