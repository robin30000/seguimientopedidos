(function () {
    "use strict";
    angular.module("seguimientopedidos").controller("GraficoQuejasCtrl", GraficoQuejasCtrl);
    GraficoQuejasCtrl.$inject = ["$scope", "$http", "$rootScope", "$route", "services", "$filter"];

    function GraficoQuejasCtrl($scope, $http, $rootScope, $route, services) {
        ...
        function graphic(data) {
            ...
            services.myService(datos, 'quejasGoCtrl.php', 'graphic')
                .then((respuestas) => {
                    if (respuestas.data.state) {
                        ...
                        $scope.myChart = new Chart(ctx, {
                            ...
                            options: {
                                ...
                                title: {
                                    display: true,
                                    text: 'Grafico QuejasGo (Consolidado: ' + total + ')',
                                    position: 'top',
                                },
                                datalabels: {
                                    display: true,
        ...
        function graphicDetails(data) {
            ...
            services.myService(datos, 'quejasGoCtrl.php', 'graphicDetails')
                .then((respuestas) => {
                    if (respuestas.data.state) {
                        ...
                        function asignarColor(gestion_asesor) {
                            switch (gestion_asesor) {
                                case "Atención":
                                    return "rgb(55, 200, 70)"
                                case "Contactabilidad":
                                    return "rgb(0, 0, 85)"; // Color para "No funciono"
                                case "Sin Gestión":
                                    return "rgb(240, 120, 30)"; // Color para "Aprovisionado por contingencia"
                                default:
                        ...
                        data.forEach(item => {
                            item.color = asignarColor(item.gestion)
                        });
        ...
        function gestionPorHora(data) {
            ...
            services.myService(datos, 'quejasGoCtrl.php', 'gestionPorHora').then((data) => {
                $scope.loading = 0;
