(function () {
    ...
    function etpCtrl($scope, $rootScope, services, $route, $sce, $cookies, $location, $uibModal, $log, $interval) {
        ...
        function gestionETP(data) {
            if (data === '' || data === undefined) {
                let data = {'page': '', 'size': '', 'data': $scope.ETP}
            }
    ...
    function RegistrosETPCtrl($scope, $rootScope, services, $route, $sce, $cookies, $location, $uibModal, $log, $interval) {
        ...
        $scope.registrosETPRegistros = (data) => {
            if (!data) {
                Swal({
                    type: 'error',
                    title: 'Oppss',
            ...
            let datos = {'page': $scope.currentPage, 'size': $scope.pageSize, 'data': data}
            gestionETPTerminadoRegistros(datos);
        };
        $scope.detallePedidoRegistros = (data) => {
            if (!data) {
                Swal({
                    type: 'error',
                    title: 'Oppss',
        ...
        $scope.recargaPageRegistros = () => {
            let datos = {'page': $scope.currentPage, 'size': $scope.pageSize}
            gestionETPTerminadoRegistros(datos);
        };
        $scope.pageChangedRegistro = function () {
            let data = {'page': $scope.currentPage, 'size': $scope.pageSize, 'data': $scope.toip}
        ...
        $scope.csvETPRegistros= (data) => {
            ...
            services.myService(datos, 'etpCtrl.php', 'datosTerminadosRegistros').then((data) => {
                ...
                if (data.data.state) {
                    ...
                    var ws = XLSX.utils.json_to_sheet(data.data.data);
                    XLSX.utils.book_append_sheet(wb, ws, 'ETP');
                    XLSX.writeFile(wb, 'RegistrosFinalizadosETP_' + tiempo + '.xlsx');
                } else {
                    Swal({
                        type: 'error',
                        title: 'Opps..',
