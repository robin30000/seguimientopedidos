...
app.service('cargaRegistros', ['$http', '$cookieStore', '$q', function ($http, $cookieStore, $q) {
    this.uploadFileToUrl = function (file, uploadUrl, login) {
        ...
        $http.post('services/cargaRegistros', fd, {
            //$http.post('api/class/test/cargaRegistros', fd, {
            withCredentials: false,
            transformRequest: angular.identity,
            headers: {'Content-Type': undefined},
            params: {'user': user},
...
// Turnos
    ...
    obj.BuscarSoporteGpon = function (pedido) {
        var data = {
            method: 'BuscarSoporteGpon', data: {'pedido': pedido}
        }
        return $http.post(serviceBase1 + 'soporteGponCtrl.php', data);
    };
    ...
    obj.registrossoportegpon = function (curPage, pageSize, sort) {
        var data = {
...
app.controller('registrosCtrl', function ($scope, $http, $rootScope, $location, $route, $routeParams, $cookies, $timeout, services, cargaRegistros) {
    ...
    $scope.BuscarRegistros = function (datos) {
        ...
        function complete(data) {
            if (data.data.state == 0) {
                swal({
                    type: 'error',
                    text: 'El pedido no tiene documentacion'
                })
            } else {
                var datos = data.data.data;
                var counter = data.data.counter;
    ...
    $scope.csvRegistros = function () {
        ...
        if (TotalDays > 31) {
            swal({
                type: 'error',
                text: 'Para optimizacion de los reportes estos no pueden sobrepasar los 31 dias'
            })
        } else if ($scope.Registros.fechaini == '' || $scope.Registros.fechaini == undefined) {
            swal({
                type: 'error',
                text: 'Ingrese la fecha inicial'
            })
    ...
    $scope.uploadFile = function () {
        if ($scope.myFile == undefined || $scope.myFile == '') {
            ...
            promise = cargaRegistros.uploadFileToUrl(file, uploadUrl, $scope.user)
...
app.controller('GestioncontingenciasCtrl', function ($http, $scope, $rootScope, $location, $route, $routeParams, $cookies, $cookieStore, $timeout, services, fileUpload) {
    $scope.rutaCierreMasivoContin = "partial/modals/cierreMasivoContingencias.html";
    $scope.haypedidoOtros = false;
    $scope.haypedidoTV = false;
    $scope.loadingData = false;
    ...
    $scope.bb8 = function (pedido) {
        ...
        $scope.url = "http://10.100.66.254:8080/BB8/contingencias/Buscar/GetClick/" + pedido; /*datos click*/
        $http.get($scope.url, {timeout: 4000})
            .then(function (data) {
                console.log(data, ' clic ');
                ...
                $scope.url = "http://10.100.66.254:8080/BB8/contingencias/Buscar/GetPlanBaMSS/" + pedido; /*internet*/
                $http.get($scope.url, {timeout: 4000})
                    .then(function (data) {
                            ...
                            if (data.data.length > 0) {
                                ...
                                $scope.url = "http://10.100.66.254:8080/BB8/contingencias/Buscar/GetPlanBaMSS/" + $scope.clic.UNECodigoDireccionServicio; /*internet*/
                                $http.get($scope.url, {timeout: 4000})
                                    .then(function (data) {
                            ...
                            $scope.url = "http://10.100.66.254:8080/BB8/contingencias/Buscar/GetPlanTOMSS/" + pedido /*telefonia*/
                            $http.get($scope.url, {timeout: 4000})
                                .then(function (data) {
                                    ...
                                    if (data.data.length > 0) {
                                        ...
                                        $scope.url = "http://10.100.66.254:8080/BB8/contingencias/Buscar/GetPlanTOMSS/" + $scope.clic.UNECodigoDireccionServicio /*telefonia*/
                                        $http.get($scope.url, {timeout: 4000})
                                            .then(function (data) {
                                    ...
                                    $scope.url = "http://10.100.66.254:8080/BB8/contingencias/Buscar/GetPlanTVMSS/" + pedido /*tv*/
                                    $http.get($scope.url, {timeout: 4000})
                                        .then(function (data) {
                                            console.log(data, ' tv');
                                            if (data.data.length > 0) {
                                                ...
                                                $scope.url = "http://10.100.66.254:8080/BB8/contingencias/Buscar/GetPlanTVMSS/" + $scope.clic.UNECodigoDireccionServicio /*tv*/
                                                $http.get($scope.url, {timeout: 4000})
                                                    .then(function (data) {
...
//  }
    ...
    $scope.descargarContingencias = function (fechaInicial, fechafinal) {
        ...
        let date_1 = new Date(fechaInicial);
        let date_2 = new Date(fechafinal);
        let diff = date_2 - date_1;
        let TotalDays = Math.ceil(diff / (1000 * 3600 * 24));
        if (TotalDays > 31) {
            swal({
...
app.controller('GestionsoportegponCtrl', function ($scope, $rootScope, $location, $route, $routeParams, $cookies, $cookieStore, $timeout, services) {
    ...
    $scope.BuscarSoporteGpon = function () {

        services.BuscarSoporteGpon($scope.gpon.pedido).then(complete).catch(failed)
        ...
        function complete(data) {
            ...
            if (data.data.state == 99) {
                ...
                $scope.gpon.databsucarPedido = data.data.data;
                $('#modalPedidoGpon').modal('show');
                return data.data;
            }
        }
...
app.controller('registrossoportegponCtrl', function ($scope, $http, $rootScope, $location, $route, $routeParams, $cookies, $timeout, services, cargaRegistros) {
    ...
    $scope.muestraNotas = function (row) {
        ...
        $scope.pedido = row.entity.unepedido;
        $scope.tarea = row.entity.tarea;
        $scope.velocidadnavegacion = row.entity.velocidad_navegacion;
        $scope.arpon = row.entity.arpon;
        $scope.nap = row.entity.nap;
        $scope.hilo = row.entity.hilo;
        $scope.intenet1 = (row.entity.port_internet_1 == '1') ? 'X' : '';
        $scope.intenet2 = (row.entity.port_internet_2 == '1') ? 'X' : '';
        $scope.intenet3 = (row.entity.port_internet_3 == '1') ? 'X' : '';
        $scope.intenet4 = (row.entity.port_internet_4 == '1') ? 'X' : '';
        $scope.television1 = (row.entity.port_television_1 == '1') ? 'X' : '';
        $scope.television2 = (row.entity.port_television_2 == '1') ? 'X' : '';
        $scope.television3 = (row.entity.port_television_3 == '1') ? 'X' : '';
        $scope.television4 = (row.entity.port_television_4 == '1') ? 'X' : '';

        let listaseriales = row.entity.serial.split(',');
        let listamacs = row.entity.mac.split(',');

        $scope.listaSeriales = listaseriales;
        $scope.listaMacs = listamacs;
        $scope.observaciones = row.entity.observacion;
...
app.controller('GestioncodigoincompletoCtrl', function ($scope, $rootScope, $location, $route, $routeParams, $cookies, $cookieStore, $timeout, services) {
    ...
    $scope.gestionarCodigoIncompleto = function (row) {
        ...
        if (row.entity.Tipificacion == undefined || row.entity.Tipificacion == '') {
            Swal(
                'Error',
        ...
        if (datos.fechafin < datos.fechaini) {
            ...
            services.expCsvRegistrosCodigoIncompleto(datos).then(
                function (datos) {
                    ...
                    var array = typeof data != 'object' ? JSON.parse(data) : data;
                    var str = '';
                    var column = `ID_CODIGO_INCOMPLETO|| TAREA|| NUMERO_CONTACTO|| NOMBRE_CONTACTO|| UNEPEDIDO|| TASKTYPECATEGORY|| UNEMUNICIPIO|| UNEPRODUCTOS|| ENGINEER_ID|| ENGINEER_NAME|| MOBILE_PHONE|| FECHA_SOLICITUD|| FECHA_RESPUESTA|| OBSERVACION|| RESPUESTA_GESTION \r\n`;
                    str += column;
                    for (var i = 0; i < array.length; i++) {
                        var line = '';
                        for (var index in array[i]) {
...
app.run(['$rootScope', 'services', function ($rootScope, services) {
    ...
    $rootScope.conceptosRegistros = [
        {ID: '', CONCEPTO: '--Seleccione--'}, {ID: 'pedido', CONCEPTO: 'Pedido'},
        {ID: 'asesor', CONCEPTO: 'Asesor'}, {ID: 'accion', CONCEPTO: 'Accion'}, {ID: 'proceso', CONCEPTO: 'Proceso'}
        /*{
            ID: 'piloto',
            CONCEPTO: 'Piloto'
        },
